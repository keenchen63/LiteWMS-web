...
import { 
  ...
  ArrowRight, Save, Pencil, List, History, Calendar, Search, ShoppingCart, User, FileText, RotateCcw
} from 'lucide-react';
...
interface CategoryCardProps {
  category: Category;
  onAdd: (category: Category, specs: Record<string, string>, quantity: number) => void;
}

const CategoryCard: React.FC<CategoryCardProps> = ({ category, onAdd }) => {
  ...
  const handleAdd = (e: React.MouseEvent) => {
    ...
    // 验证所有属性都已填写
    const allFilled = category.attributes.every(attr => {
      const value = specs[attr.name];
    ...
    >
      ...
      {!isExpanded ? (
        <div className="text-sm text-slate-600">
          ...
          <div className="mt-3 text-blue-600 text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
            <Plus size={14} /> 点击填写
          </div>
        </div>
      ) : (
        <div className="space-y-3" onClick={(e) => e.stopPropagation()}>
          <div className="space-y-2">
            {category.attributes.map(attr => {
              ...
              return (
                <div key={attrName}>
                  <label className="block text-xs font-medium text-slate-600 mb-1">{attrName}</label>
                  {attrOptions && attrOptions.length > 0 ? (
                    <select
                      required
                      className="w-full px-2 py-1.5 bg-white border border-blue-300 rounded-lg text-sm focus:border-blue-500 outline-none"
                      onChange={(e) => setSpecs({...specs, [attrName]: e.target.value})}
                      value={specs[attrName] || ''}
          ...
          <div className="pt-2 border-t border-blue-200">
            ...
            <input
              ...
              onClick={(e) => e.stopPropagation()}
              onFocus={(e) => e.stopPropagation()}
              className="w-full px-2 py-1.5 border border-blue-300 rounded-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none text-sm font-bold bg-white"
            />
          </div>
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button
              ...
              onClick={handleAdd}
              disabled={!allFilled}
              className="flex-1 px-3 py-2 text-sm text-white bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed rounded-lg transition-colors flex items-center justify-center gap-1"
            >
              <Plus size={14} /> 添加
            </button>
          </div>
